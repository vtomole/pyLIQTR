...
class InnerPrepare(qt.GateWithRegisters):
    ...
    def selection_registers(self) -> Tuple[qt._infra.registers.Register]:
        contiguous_reg = qt._infra.registers.Register(name="contiguous_index", dtype=BQUInt(bitsize=self.n_LXi+1),side=Side.RIGHT)
        p_reg = qt._infra.registers.Register(name="p",dtype=BQUInt(bitsize=self.n_Xi))
